...
bool DeliveryData::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t(U("failed"))))
    {
        ...
        if(!fieldValue.is_null())
        {
            int32_t refVal_failed;
            ok &= ModelBase::fromJson(fieldValue, refVal_failed);
            setFailed(refVal_failed);
        }
    }
    if(val.has_field(utility::conversions::to_string_t(U("errored"))))
    {
        ...
        if(!fieldValue.is_null())
        {
            int32_t refVal_errored;
            ok &= ModelBase::fromJson(fieldValue, refVal_errored);
            setErrored(refVal_errored);
        }
    }
    if(val.has_field(utility::conversions::to_string_t(U("converted"))))
    {
        ...
        if(!fieldValue.is_null())
        {
            int32_t refVal_converted;
            ok &= ModelBase::fromJson(fieldValue, refVal_converted);
            setConverted(refVal_converted);
        }
    }
...
bool DeliveryData::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    ...
    {
        int32_t refVal_failed;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("failed"))), refVal_failed );
        setFailed(refVal_failed);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(U("errored"))))
    {
        int32_t refVal_errored;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("errored"))), refVal_errored );
        setErrored(refVal_errored);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(U("converted"))))
    {
        int32_t refVal_converted;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("converted"))), refVal_converted );
        setConverted(refVal_converted);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(U("received"))))
    {
        int32_t refVal_received;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("received"))), refVal_received );
        setReceived(refVal_received);
    }
    return ok;
}
