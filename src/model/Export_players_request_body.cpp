...
bool Export_players_request_body::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t(U("extra_fields"))))
    {
        ...
        {
            std::vector<utility::string_t> refVal_extra_fields;
            ok &= ModelBase::fromJson(fieldValue, refVal_extra_fields);
            setExtraFields(refVal_extra_fields);
        }
    }
    if(val.has_field(utility::conversions::to_string_t(U("last_active_since"))))
    {
        ...
        if(!fieldValue.is_null())
        {
            int32_t refVal_last_active_since;
            ok &= ModelBase::fromJson(fieldValue, refVal_last_active_since);
            setLastActiveSince(refVal_last_active_since);
        }
    }
    if(val.has_field(utility::conversions::to_string_t(U("segment_name"))))
    {
        ...
        if(!fieldValue.is_null())
        {
            utility::string_t refVal_segment_name;
            ok &= ModelBase::fromJson(fieldValue, refVal_segment_name);
            setSegmentName(refVal_segment_name);
        }
    }
    return ok;
}
...
bool Export_players_request_body::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    ...
    {
        std::vector<utility::string_t> refVal_extra_fields;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("extra_fields"))), refVal_extra_fields );
        setExtraFields(refVal_extra_fields);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(U("last_active_since"))))
    {
        int32_t refVal_last_active_since;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("last_active_since"))), refVal_last_active_since );
        setLastActiveSince(refVal_last_active_since);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(U("segment_name"))))
    {
        utility::string_t refVal_segment_name;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("segment_name"))), refVal_segment_name );
        setSegmentName(refVal_segment_name);
    }
    return ok;
}
