...
bool Notification200Errors::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t(U("invalid_external_user_ids"))))
    {
        ...
        {
            std::vector<utility::string_t> refVal_invalid_external_user_ids;
            ok &= ModelBase::fromJson(fieldValue, refVal_invalid_external_user_ids);
            setInvalidExternalUserIds(refVal_invalid_external_user_ids);
        }
    }
    if(val.has_field(utility::conversions::to_string_t(U("invalid_player_ids"))))
    {
        ...
        if(!fieldValue.is_null())
        {
            std::vector<utility::string_t> refVal_invalid_player_ids;
            ok &= ModelBase::fromJson(fieldValue, refVal_invalid_player_ids);
            setInvalidPlayerIds(refVal_invalid_player_ids);
        }
    }
    return ok;
}
...
bool Notification200Errors::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    ...
    {
        std::vector<utility::string_t> refVal_invalid_external_user_ids;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("invalid_external_user_ids"))), refVal_invalid_external_user_ids );
        setInvalidExternalUserIds(refVal_invalid_external_user_ids);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(U("invalid_player_ids"))))
    {
        std::vector<utility::string_t> refVal_invalid_player_ids;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("invalid_player_ids"))), refVal_invalid_player_ids );
        setInvalidPlayerIds(refVal_invalid_player_ids);
    }
    return ok;
}
